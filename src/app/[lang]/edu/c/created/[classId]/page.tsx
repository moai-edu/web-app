...
export default async function Page({ params }: Props) {
    ...
    const _class = await domain.getById(classId)
    if (!_class || _class.userId !== session.user.id) {
        return <div>Not authenticated</div>
    }

    const courseDomain = new CourseDomain()
    const course = await courseDomain.getCourseUnitsAndTiles(session.user!.slug!, _class.courseId)
    if (!course) {
        ...
        <div className="flex justify-center gap-3 pt-14 sm:p-6 sm:pt-10 lg:gap-12">
            <div className="flex max-w-2xl grow flex-col">
                {units.map((unit, index) => (
                    <UnitSection id={classId} units={units} unit={unit} key={index} />
                ))}
