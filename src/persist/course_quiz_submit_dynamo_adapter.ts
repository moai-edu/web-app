...
export function CourseQuizSubmitDynamoAdapter(
    ...
    return {
        async create(model: CourseQuizSubmit): Promise<CourseQuizSubmit> {
            await dao.createWithGSI123(
                model,
                model.userJoinClassId,
                model.quizId,
                model.classId
            )
            return model
        ...
        async getByUserJoinClassIdAndQuizId(
            userJoinClassId: string,
            quizId: string
        ): Promise<CourseQuizSubmit | null> {
            //TODO: 这里查询可以优化吗？因为这里userJoinClassId和quizId都作为GSI1和GSI2，能不能一条查询就搞定？
            const all_user_quiz_submits =
                await dao.getListByGSI1<CourseQuizSubmit>(userJoinClassId)
            return (
                all_user_quiz_submits.find(
                    (submit) => submit.quizId === quizId
                ) || null
            )
        },
        ...
        async getListByUserJoinClassId(
            userJoinClassId: string
        ): Promise<CourseQuizSubmit[]> {
            return dao.getListByGSI1<CourseQuizSubmit>(userJoinClassId)
        }
    }
}
